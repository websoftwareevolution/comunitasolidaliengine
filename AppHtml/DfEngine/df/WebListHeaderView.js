...
afterRender : function(eList){
    ...
    this.changeColumnReorderingState(this.oL.pbAllowColumnReordering); // Will also (re)render the Header Menu.
},

// This destroys the header menu if it has to, and creates it afterwards.
recreateHeaderMenu : function () {
    this.destoryHeaderMenu();
    this.createHeaderMenu();
},

destoryHeaderMenu : function () {
    if (this._oHeaderMenuColumnList !== null) {
        this._oHeaderMenuColumnList.destroy();
        this._oHeaderMenuColumnList = null;
    }

    if (this._oHeaderMenuRestore !== null) {
        this._oHeaderMenuRestore.destroy();
        this._oHeaderMenuRestore = null;
    }

    if (this._oHeaderMenu !== null) {
        this._oHeaderMenu.destroy();
        this._oHeaderMenu = null;
    }
},

createHeaderMenu : function () {
    // Return if icompatible with the current device/browser-state.
    if (df.sys.isMobile ||
        (df.sys.isIE && df.sys.iVersion < 11)) return;

    // If it should not have any purpose.
    if (!this.oL.pbAllowColumnReordering && !this.oL.pbAllowColumnHiding) return;

    let oWebApp = this.oL.getWebApp(); // For getting translations.

    this._oHeaderMenu = new df.WebContextMenu("ShowHideMenu", this.oL);
    this._oHeaderMenu.set_psControlName(this.oL.getLongName());
    this._oHeaderMenu.set_psContextCSSSelector(".WebList_Head");
    ...
    this._oHeaderMenuRestore = this._oHeaderMenu.createMenuItem(oWebApp.getTrans("weblist_HeaderMenuRestoreMenuName"));
    this._oHeaderMenuRestore.OnClick.on(function () {
    ...
    this._oHeaderMenu.render();
    this._oHeaderMenu.afterRender();
    this.rebuildHeaderMenu();
...
rebuildHeaderMenu : function() {
    var oL = this.oL;
    var that = this;

    if (!this._oHeaderMenuColumnList) return;

    // Clear any children.
    this._oHeaderMenuColumnList._aChildren = [];

    for (let i = 0; i < this.oL._aColumns.length; i++){
        if (!this.oL._aColumns[i].pbRender || this.oL._aColumns[i].psCaption.length === 0) continue;

        // Create a checked item if pbHidden is true.
        let oItem = this._oHeaderMenu.createCheckedMenuItem(
            this.oL._aColumns[i].psCaption,
            this.oL._aColumns[i].pbHidden == false
        );

        // Set a reference
        oItem._oColumnRef = this.oL._aColumns[i];

        // Should a click be performed:
        oItem.OnClick.on(function (oEv) {
            let aFilteredVisible = that.oL._aColumns.filter(function (oCol) {
        ...
        this._oHeaderMenuColumnList.addChild(oItem);
    }
...
headerHtml : function(aHtml){
    ...
    for(let i = 0; i < this.oL._aColumnMapper.length; i++){
        ...
        aHtml.push('><div>');
        aHtml.push('<div class="WebList_ColCap">', (oCol.psCaption && oCol.pbShowCaption ? oCol.psCaption : '&nbsp;'), '</div>');
...
updateHeader : function(){
    ...
    if (this.oL.pbAllowColumnHiding) this.rebuildHeaderMenu();
...
moveColumn : function(eColumnHeader, oEv, iOriginalDbIndex) {
    ...
    eColumnGhost.style.position = "absolute";
    ...
    document.body.appendChild(eColumnGhost);
    ...
    function onStopMove(oEv){
        ...
        df.dom.off("mouseup", window, onStopMove);
        df.dom.off("mousemove", window, onMove);

        // Remove the ghost from the view.
        document.body.removeChild(eColumnGhost);
